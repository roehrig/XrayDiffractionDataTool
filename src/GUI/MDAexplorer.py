...
class TopFrame(wx.Frame):
    ...
    def readData(self, path):
        ...
        if (USE_OLD_PUBLISHER):
            try:
                self.dataPublisher.sendMessage(("new_scan_dimensions"), (self.data[2].curr_pt, self.data[1].curr_pt), onTopicNeverCreated=None)
            except IndexError:
                self.dataPublisher.sendMessage(("new_scan_dimensions"), self.data[2].curr_pt, onTopicNeverCreated=None)
        else:
            try:
                self.dataPublisher.sendMessage(("new_scan_dimensions"), message=(self.data[2].curr_pt, self.data[1].curr_pt))
            except IndexError:
                self.dataPublisher.sendMessage(("new_scan_dimensions"), message=self.data[2].curr_pt)
