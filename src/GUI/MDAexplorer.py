...
def calcRowsColumns(numPlots):
    if (numPlots == 0):
        return (0,0)
    columns = int(math.sqrt(numPlots)+0.5)
...
#!                if ininstance(data[3].p[posAxis].data, list):
                if isinstance(data[3].p[posAxis].data, list):
...
#!            print int(event.xdata + 0.4), int(event.ydata + 0.4)
            if (USE_OLD_PUBLISHER):
                self.dataPublisher.sendMessage(("new_pixel_selected"), (self.frame.data[1].curr_pt,int(event.xdata + 0.4), int(event.ydata + 0.4)))
            else:
                self.dataPublisher.sendMessage(("new_pixel_selected"), message=(self.frame.data[1].curr_pt,int(event.xdata + 0.4), int(event.ydata + 0.4)))
...
#!                    self.Bind(wx.EVT_MENU, self.OnPopupTwo, id=self.popupTwoID)
                ...
                # Get the list and list-entry number of the plot user clicked on 
...
class TopFrame(wx.Frame):
    ...
    def readData(self, path):
        ...
        if (USE_OLD_PUBLISHER):
            try:
                self.dataPublisher.sendMessage(("new_scan_dimensions"), (self.data[1].curr_pt, self.data[2].curr_pt), onTopicNeverCreated=None)
            except IndexError:
                self.dataPublisher.sendMessage(("new_scan_dimensions"), self.data[1].curr_pt, onTopicNeverCreated=None)
        else:
            try:
                self.dataPublisher.sendMessage(("new_scan_dimensions"), message=(self.data[1].curr_pt, self.data[2].curr_pt))
            except IndexError:
                self.dataPublisher.sendMessage(("new_scan_dimensions"), message=self.data[1].curr_pt)
